...
exports.default = function () {
    ...
    function getMessageDescriptorKey(path) {
        ...
        console.warn(path.buildCodeFrameError('[React Intl] Skipping, messages must be statically evaluate-able for extraction.'));
    ...
    var resourceFiles = void 0;

    // It is not always the case that `state.opts` is defined while traversing.
    // Did not manage to find out the cause.
    var loadRefs = function loadRefs(state) {
        if (!references && state.opts.resourceFiles) {
            references = state.opts.resourceFiles.map(function (path) {
                return JSON.parse((0, _fs.readFileSync)(path));
            });
...
var COMPONENT_NAMES = ['FormattedMessage', 'FormattedHTMLMessage', 'Msg'];

var FUNCTION_NAMES = ['defineMessages', 'msg'];
